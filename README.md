...
- Bug real corregido + skin nueva "Aero" (`v2.13.0`): el menú de selección de skin del gadget (botón de paleta o clic derecho) se cerraba solo cada ~1 segundo, antes de poder elegir una opción -- causado por la reafirmación periódica de "siempre encima" del gadget, que en Windows descarta cualquier menú nativo abierto en ese instante. Ya no se auto-cierra. De paso, el modo gadget suma una 5ta apariencia, "Aero" (estética "Frutiger Aero": vidrio aqua/teal con reflejos casi blancos). Ver `SDD.md` (`v2.13.0`) para el detalle completo, incluyendo dos paletas de color candidatas que se probaron y descartaron antes de la final.
- Temas de toda la app (`v2.14.0`): las 5 apariencias del modo gadget (Clásico/Cristal/Claro/Neón/Aero) ya no son solo del gadget -- un botón de ícono nuevo en el encabezado (junto al del gadget) recolorea de una sola vez el encabezado, el formulario, la lista de reuniones, el calendario mensual, la vista semanal y el modo gadget, todos desde el mismo ajuste (`appTheme`, antes `gadgetSkin` -- un usuario que ya había elegido una skin conserva esa misma elección al actualizar). "Clásico" se ve pixel-idéntico a la app anterior a esta versión; "Cristal" y "Aero" (los temas translúcidos) ahora vuelven translúcida la ventana completa, no solo el panel del gadget. Ver `SDD.md` (`v2.14.0`) para el detalle completo, incluyendo un bug real de contraste encontrado y corregido durante el desarrollo (la paleta "Aero").
...
- Auto-scroll a la hora actual en la vista semanal (`v2.14.1`): antes, la cuadrícula siempre abría con el desplazamiento fijo en 00:00, obligando a bajar manualmente para ver las reuniones de la mañana y la línea de "ahora". Ahora, cada vez que entras a la vista semanal desde otra vista (si la semana mostrada es la actual) o pulsas "Esta semana", el desplazamiento se centra en la hora actual con un pequeño margen arriba (igual que Teams/Outlook) -- nunca en cada minuto del reloj, así que si te desplazas manualmente después, nunca se te "arrebata" esa posición.
- Duración de reunión, estilo Teams (`v2.15.0`): el formulario suma un campo "Duración" (combobox editable junto a fecha/hora, con 15/30/45/60/90/120 minutos como opciones rápidas y cualquier valor personalizado aceptado). La vista de calendario mensual muestra el rango completo ("09:00-09:30 Título") en vez de solo la hora de inicio. Las reuniones guardadas antes de esta versión, sin duración explícita, se muestran con 30 minutos por defecto -- no requiere ninguna migración de datos.
- Bloques de reunión de color completo en la vista semanal, look real de Teams (`v2.16.0`): reemplaza la barra delgada de `v2.15.0` por un bloque de color a todo lo alto de la duración de la reunión, con el título dibujado adentro (no solo una barra decorativa). Reuniones simultáneas dividen el ancho de la columna del día en partes iguales (hasta 2 reuniones con bloque propio; la 3ra+ se fusiona en un chip agregado "+N más" en vez de quedar sin representación); clic selecciona y clic derecho abre el mismo menú "Editar"/"Eliminar"/"Eliminar serie completa" de siempre, ahora directamente sobre el bloque. La vista de calendario mensual no cambia (ya mostraba cada entrada como un chip de color con el título adentro desde antes).
- Rediseño visual de la ventana de alarma (`v2.16.1`): corrige un bug real -- el botón "Silenciar alarma" usaba el mismo color que una de las dos fases del parpadeo de fondo, así que se volvía ilegible durante la mitad de cada ciclo. Se separó el halo parpadeante (borde exterior) del contenido (tarjeta interior de fondo fijo), así ningún texto ni botón cambia de color bajo el cursor. También se unificó la etiqueta de "Recordatorio"/"Inicio" en una sola insignia, se agregó una franja de acento que pulsa en sincronía con el halo, y se ordenó la jerarquía tipográfica (insignia, título, reunión, hora, aviso). Sin cambios de comportamiento: mismos botones, mismo parpadeo del título, misma cola de alarmas.
- Contador en vivo, indicador pulsante y atajos de teclado en la ventana de alarma (`v2.17.0`): la alarma ahora muestra "Empieza en MM:SS" (o "Inició hace MM:SS" tras la hora de inicio), actualizado cada segundo, y una línea "● ALARMA ACTIVA" que pulsa en sincronía con la franja de acento. Enter abre Teams (si hay enlace guardado), Alt+S silencia la alarma, y Esc ya no puede cerrarla por accidente. Los botones crecieron a una altura mínima de ~45px. Sin cambios en el sonido, el parpadeo, la cola de alarmas ni la lógica de horarios.
- Bug real corregido: dos empresas podían quedar con el mismo color a simple vista (`v2.17.1`). El color por trabajo/empresa en las vistas de lista, calendario y semana ya existía, pero se basaba en un hash del nombre que no garantizaba separación entre nombres distintos -- con los datos reales de un usuario, "SRS" y "Direct English" caían en el mismo rosa. Ahora los colores se asignan por ángulo dorado sobre el conjunto de empresas guardadas, garantizando que todas se vean claramente distintas entre sí.
- Tooltip al pasar el mouse sobre un bloque de reunión en la vista semanal (`v2.18.0`): muestra el título y la fecha/hora exacta de inicio y fin (ej. "10 ago 2026, 09:30 - 10:00"), útil cuando el bloque es muy angosto o corto para mostrar toda esa información en su propio texto. No aplica al chip agregado "+N más" (sigue sin ser interactivo). Sin cambios en el clic, clic derecho, ni en el resto del comportamiento de la vista semanal.
- "Editar serie completa" en el menú contextual de calendario y semana (`v2.19.0`): junto a "Eliminar serie completa" (disponible bajo la misma condición: 2+ ocurrencias vivas de la serie), abre el mismo formulario de edición pero aplica los cambios de título, empresa, hora, duración, sonido, enlace de Teams y notas a TODAS las ocurrencias de la serie a la vez -- cada ocurrencia conserva su propia fecha (solo la hora del día se replica en todas), y la fecha/el tipo de repetición/la cantidad de ocurrencias quedan bloqueados en el formulario mientras se edita la serie, ya que no aplican. El botón "Guardar" cambia a "Actualizar serie" para que quede claro que el cambio afecta a toda la serie, no solo a la reunión que se clicó.
- Campo de duración mejorado (`v2.19.1`): el combobox de duración ahora ofrece opciones cada 5 minutos (5 a 120) en vez de solo 15/30/45/60/90/120, para que un valor común como 80 ya aparezca en el desplegable -- sigue aceptando cualquier valor escrito a mano, igual que antes. De paso, ya no deja escribir letras ni caracteres raros en ese campo (valida dígito por dígito mientras se escribe); la validación de rango al guardar (5 a 1440 minutos) no cambia.
- Bug real corregido: la línea/punto de "hora actual" de la vista semanal podían quedar completamente tapados detrás de un bloque de reunión (`v2.20.0`). Ahora se reafirman siempre por encima. De paso, colores configurables: el color de esa línea y el color de cada empresa (antes solo automático, distribuido por ángulo dorado) ahora se pueden personalizar desde "Gestionar empresas", con un botón "Restablecer" por empresa para volver al color automático.
- Splash animado al iniciar (`v2.21.0`): la pantalla de carga ya no es solo el texto "Cargando TimerMeet…" -- ahora reproduce arriba de ese texto una animación GIF corta (`assets/pingpong_loading.gif`, se reproduce una vez en ~1.2 segundos) antes de mostrar la ventana real. Si el archivo faltara o no pudiera leerse, la app arranca igual, sin animación, sin ningún retraso ni error visible. No aplica al modo gadget (ya arranca directo, sin ninguna pantalla de carga, igual que antes).
